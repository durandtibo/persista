...
@pytest.fixture
def store() -> Generator[TypedSQLiteStore, None, None]:
...
) -> Generator[TypedSQLiteStore, None, None]:
...
def test_init_defaults_to_in_memory() -> None:
    with TypedSQLiteStore() as store:
        assert store.count() == 0


def test_init_accepts_sqlite_connect_kwargs() -> None:
    with TypedSQLiteStore(":memory:", timeout=5.0) as store:
        assert store.count() == 0


def test_init_no_schema_stores_everything_in_extra(store: TypedSQLiteStore) -> None:
...
def test_from_path_memory_uses_shared_cache_uri() -> None:
    with TypedSQLiteStore.from_path(":memory:") as store:
        assert store.count() == 0


def test_from_path_read_only_can_read_existing_data(store_read_only: TypedSQLiteStore) -> None:
    assert store_read_only.count() == 4


def test_from_path_read_only_rejects_writes(store_read_only: TypedSQLiteStore) -> None:
...
def test_init_read_only_connection_with_existing_table_does_not_raise(
    store_read_only: TypedSQLiteStore, items: dict[str, dict[str, Any]]
) -> None:
    with store_read_only:
        assert store_read_only.count() == len(items)
...
def test_init_read_only_connection_without_existing_table_swallows_operational_error(
    ...
    with (
        TypedSQLiteStore.from_path(path, read_only=True) as store,
...
def test_repr(store: TypedSQLiteStore) -> None:
    assert repr(store).startswith("TypedSQLiteStore(")


def test_str(store: TypedSQLiteStore) -> None:
    assert str(store).startswith("TypedSQLiteStore(")


def test_repr_after_close_does_not_raise(store: TypedSQLiteStore) -> None:
    store.close()
    assert repr(store).startswith("TypedSQLiteStore(")
...
def test_set_increases_count(store: TypedSQLiteStore) -> None:
    store.set("1", {"text": "hello"})
    assert store.count() == 1


def test_set_stores_value(store: TypedSQLiteStore) -> None:
    store.set("1", {"text": "hello"})
    assert store.get("1") == {"text": "hello"}


def test_set_default_overwrites_existing(store: TypedSQLiteStore) -> None:
...
def test_set_on_conflict_raise(store: TypedSQLiteStore) -> None:
    store.set("1", {"text": "original"})
...
def test_set_on_conflict_skip(store: TypedSQLiteStore) -> None:
    store.set("1", {"text": "original"})
    store.set("1", {"text": "updated"}, on_conflict="skip")
    assert store.get("1") == {"text": "original"}


def test_set_on_conflict_overwrite(store: TypedSQLiteStore) -> None:
    store.set("1", {"text": "original"})
    store.set("1", {"text": "updated"}, on_conflict="overwrite")
    assert store.get("1") == {"text": "updated"}


def test_set_on_conflict_merge(store: TypedSQLiteStore) -> None:
...
def test_set_on_conflict_new_key_is_unaffected(store: TypedSQLiteStore) -> None:
    store.set("1", {"text": "hello"}, on_conflict="raise")
    assert store.get("1") == {"text": "hello"}


def test_set_on_conflict_invalid_raises(store: TypedSQLiteStore) -> None:
...
def test_set_many_increases_count(
    store: TypedSQLiteStore, items: dict[str, dict[str, Any]]
) -> None:
    store.set_many(items)
    assert store.count() == len(items)


def test_set_many_empty_is_no_op(store: TypedSQLiteStore) -> None:
    store.set_many({})
    assert store.count() == 0


def test_set_many_default_overwrites_existing(store: TypedSQLiteStore) -> None:
...
def test_set_many_on_conflict_raise(store: TypedSQLiteStore) -> None:
...
def test_set_many_on_conflict_skip(store: TypedSQLiteStore) -> None:
    store.set_many({"1": {"text": "original"}})
...
def test_set_many_on_conflict_overwrite(store: TypedSQLiteStore) -> None:
    store.set_many({"1": {"text": "original"}})
...
def test_set_many_on_conflict_merge(store: TypedSQLiteStore) -> None:
    store.set_many({"1": {"text": "original", "author": "Alice"}})
    store.set_many({"1": {"text": "updated"}}, on_conflict="merge")
    assert store.get("1") == {"text": "updated", "author": "Alice"}


def test_set_many_on_conflict_invalid_raises(store: TypedSQLiteStore) -> None:
    with pytest.raises(ValueError, match=r"Invalid on_conflict value"):
...
def test_set_batches_empty_is_no_op(store: TypedSQLiteStore) -> None:
    store.set_batches([])
    assert store.count() == 0


def test_set_batches_writes_all_pairs(store: TypedSQLiteStore) -> None:
    store.set_batches([("1", {"v": 1}), ("2", {"v": 2}), ("3", {"v": 3})], batch_size=2)
    assert store.count() == 3
    assert store.get("2") == {"v": 2}


def test_set_batches_consumes_a_generator(store: TypedSQLiteStore) -> None:
...
def test_set_batches_on_conflict_skip(store: TypedSQLiteStore) -> None:
    store.set("1", {"text": "original"})
...
def test_count_empty_store(store: TypedSQLiteStore) -> None:
    assert store.count() == 0


def test_count_after_set_many(store: TypedSQLiteStore, items: dict[str, dict[str, Any]]) -> None:
    store.set_many(items)
    assert store.count() == len(items)
...
def test_get_existing_value(store: TypedSQLiteStore, items: dict[str, dict[str, Any]]) -> None:
    store.set_many(items)
    assert store.get("1") == items["1"]


def test_get_missing_key_returns_none(store: TypedSQLiteStore) -> None:
...
def test_get_many_empty_list_returns_empty_list(store: TypedSQLiteStore) -> None:
...
def test_filter_empty_store_returns_empty(store: TypedSQLiteStore) -> None:
...
def test_delete_removes_value(store: TypedSQLiteStore, items: dict[str, dict[str, Any]]) -> None:
    store.set_many(items)
    store.delete("1")
...
def test_delete_nonexistent_is_silent(store: TypedSQLiteStore) -> None:
...
def test_delete_many_nonexistent_keys_are_silent(store: TypedSQLiteStore) -> None:
    store.delete_many(["99", "100"])


def test_delete_many_single_key(store: TypedSQLiteStore, items: dict[str, dict[str, Any]]) -> None:
    store.set_many(items)
    store.delete_many(["2"])
    assert store.count() == len(items) - 1
...
def test_contains_many_all_found(store: TypedSQLiteStore, items: dict[str, dict[str, Any]]) -> None:
    store.set_many(items)
...
def test_contains_many_mixed(store: TypedSQLiteStore, items: dict[str, dict[str, Any]]) -> None:
    store.set_many(items)
...
def test_contains_many_empty_input_returns_empty_lists(store: TypedSQLiteStore) -> None:
    found, missing = store.contains_many([])
    assert found == []
    assert missing == []


def test_contains_many_empty_store_returns_all_missing(store: TypedSQLiteStore) -> None:
...
def test_get_columns_info_returns_dict(store: TypedSQLiteStore) -> None:
...
def test_show_columns_info_does_not_raise(
    store: TypedSQLiteStore, caplog: pytest.LogCaptureFixture
) -> None:
...
def test_show_columns_info_output_contains_column_names(
    typed_store: TypedSQLiteStore, caplog: pytest.LogCaptureFixture
) -> None:
...
def test_show_columns_info_returns_none(store: TypedSQLiteStore) -> None:
...
def test_keys_empty_store_yields_nothing(store: TypedSQLiteStore) -> None:
    assert list(store.keys()) == []


def test_keys_returns_all_keys(store: TypedSQLiteStore, items: dict[str, dict[str, Any]]) -> None:
    store.set_many(items)
...
def test_values_empty_store_yields_nothing(store: TypedSQLiteStore) -> None:
...
def test_values_is_lazy_generator(store: TypedSQLiteStore) -> None:
...
def test_iter_batches_empty_store_yields_nothing(store: TypedSQLiteStore) -> None:
    assert list(store.iter_batches()) == []


def test_iter_batches_returns_generator(store: TypedSQLiteStore) -> None:
...
def test_iter_batches_zero_batch_size_raises(store: TypedSQLiteStore) -> None:
    with pytest.raises(ValueError, match="batch_size must be a positive integer"):
        list(store.iter_batches(batch_size=0))


def test_iter_batches_negative_batch_size_raises(store: TypedSQLiteStore) -> None:
    with pytest.raises(ValueError, match="batch_size must be a positive integer"):
        list(store.iter_batches(batch_size=-1))


def test_iter_batches_error_raised_before_any_query(store: TypedSQLiteStore) -> None:
...
def test_close_closes_underlying_connection(store: TypedSQLiteStore) -> None:
    store.close()
    with pytest.raises(sqlite3.ProgrammingError, match=r"closed database"):
        store._conn.execute("SELECT 1")


def test_close_is_idempotent(store: TypedSQLiteStore) -> None:
    store.close()
    store.close()  # should not raise


def test_close_returns_none(store: TypedSQLiteStore) -> None:
...
def test_context_manager_closes_on_exception() -> None:
    msg = "boom"
    with pytest.raises(ValueError, match="boom"), TypedSQLiteStore(":memory:") as store:
...
def test_context_manager_multiple_open_close_in_memory() -> None:
    typed_sqlite_store = TypedSQLiteStore(":memory:")
    for i in range(3):
        with typed_sqlite_store as store:
            assert store.count() == 0
...
def test_context_manager_multiple_open_close_persistent(tmp_path: Path) -> None:
    typed_sqlite_store = TypedSQLiteStore(tmp_path / "data.db")
    for i in range(3):
        with typed_sqlite_store as store:
            store.set(str(i), {"text": "hello"})
